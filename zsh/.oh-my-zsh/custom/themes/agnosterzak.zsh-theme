...
# ---- COLOR FIX: background 240 (near-black gray) + bold white text ----
# original: prompt_segment yellow magenta "...@$USER..."  (low contrast)
prompt_context() {
  if [[ -n "$SSH_CLIENT" ]]; then
    prompt_segment 22 white "%{$fg_bold[white]%(!.%{%F{white}%}.)%}$USER@%m%{$fg_no_bold[white]%}"
  else
...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ -n $dirty ]]; then
      clean=''
      # ---- COLOR FIX: dirty-but-no-conflicts state, was yellow/magenta (low contrast) ----
      bgclr='yellow'
      fgclr='black'
    else
      clean=' ✔'
...
# ---- COLOR FIX: kept cyan/white (already high contrast), no change needed ----
prompt_dir() {
  prompt_segment 24 white "%{$fg_bold[white]%}%~%{$fg_no_bold[white]%}"
}
...
# ---- COLOR FIX: was blue/white. Switched bg to 17 (deep navy, true-color-ish
#      256 index close to Tokyo Night's #1a1b26) so white text has real contrast
#      instead of riding on a mid-brightness ANSI blue. ----
prompt_time() {
  prompt_segment 17 white "%{$fg_bold[white]%}%D{%a %e %b - %H:%M}%{$fg_no_bold[white]%}"
}
